        ...
        {showSearch && user
         ?
          ...
          chatData.map((item, index) => (
            <div onClick={() => setChat(item)} key={index} className=" group flex items-center gap-[10px] p-[10px] px-[20px] cursor-pointer text-[13px] hover:bg-blue-500 transition-all duration-200 bg-gray-50 mt-0.5 rounded-xl">
              <img className={`w-[35px] aspect-[1/1] rounded-full  ${item.messageSeen || item.messageId === messagesId ? " " : " ring-3 ring-[#08b946]" }`} src={item.userData.avatar} alt="" />
